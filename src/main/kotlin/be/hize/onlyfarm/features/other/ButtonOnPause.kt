package be.hize.onlyfarm.features.other
...
class ButtonOnPause {
    private val buttonId = System.nanoTime().toInt() + 2

    @SubscribeEvent
    fun onGuiAction(event: GuiScreenEvent.ActionPerformedEvent.Post) {
        if (!HypixelUtils.onHypixel) return
        if (OnlyFarmMod.feature.other.pauseButton && event.gui is GuiIngameMenu && event.button.id == buttonId) {
    ...
    fun onGuiInitPost(event: GuiScreenEvent.InitGuiEvent.Post) {
        if (!HypixelUtils.onHypixel) return
        if (OnlyFarmMod.feature.other.pauseButton && event.gui is GuiIngameMenu) {
            ...
            event.buttonList.add(GuiButton(buttonId, x, 0.coerceAtLeast(y), 100, 20, "OnlyFarm"))
        }
    }
}
